...
def predict_output(y,u,theta,ord):
    y_hat = np.zeros(y.shape)
    for i in range(y.size-ord):
        for o in range(ord):
            y_hat[i+ord] += -theta[o]*y[i+ord-1-o] + theta[o+ord]*u[i+ord-1-o]        
    return y_hat
...
def ARX_order_cost_fesc(u,y,ord):
    Y, phi = build_Y_phi_ARX_fesc(u,y,ord)
    theta_hat = calcular_theta_min_quad(Y,phi)
    y_hat = estimate_output_fesc(y,u, theta_hat, ord)
    V = cost_func(y,y_hat)
    return V

def find_best_ARX_order_fesc(u,y,ord,thr,v):
    best_order = 0
    for o in range(ord):
        V = ARX_order_cost_fesc(u,y,o+1)
        if v: print(f'ARX {o+1}ª ordem V = {V}') 
        if o > 0:
            if ((V_ant - V) < 0 or V > V_ant*(1-thr)):
                best_order = o
...
def build_Y_phi_ed_load_fesc(u,y):
    Y_ed = y[2:]
    phi_ed = np.concatenate((-y[1:-1],-y[:-2],-y[1:-1]*y[:-2],-y[:-2]**2,np.ones(Y_ed.shape),u[:-2],u[:-2]**2), axis=1)
    return Y_ed, phi_ed

def estimate_output_ed_fesc(y,u,theta):
    y_hat_ed = np.zeros(y.shape)
    a1, a2, a3, a4, c0, c1, c2 = theta
    for i in range(y_hat_ed.size-2):
        y_hat_ed[i+2] += -a1*y_hat_ed[i+1] - a2*y_hat_ed[i] - a3*y_hat_ed[i]*y_hat_ed[i+1] - a4*y_hat_ed[i]**2 + c0 + c1*u[i] + c2*u[i]**2
    return y_hat_ed

def predict_output_ed_fesc(y,u,theta):
    y_hat_ed = np.zeros(y.shape)
    a1, a2, a3, a4, c0, c1, c2 = theta
    for i in range(y_hat_ed.size-2):
        y_hat_ed[i+2] += -a1*y[i+1] - a2*y[i] - a3*y[i]*y_hat_ed[i+1] - a4*y[i]**2 + c0 + c1*u[i] + c2*u[i]**2
    return y_hat_ed

def build_Y_phi_ARX_fesc(u,y,ord):
    aux2_y = np.zeros((y.size-ord,1))
    aux2_u_0 = np.ones((y.size-ord,1))
    aux2_u_1 = np.zeros((y.size-ord,1))
    aux2_u_2 = np.zeros((y.size-ord,1))
    for o in range(ord):
        aux_y  = -y[ord-1-o:-1-o]
        aux_u_1  = u[ord-1-o:-1-o]
        aux_u_2  = u[ord-1-o:-1-o]**2
        aux2_y = np.concatenate((aux2_y,aux_y),axis=1)
        aux2_u_1 = np.concatenate((aux2_u_1,aux_u_1),axis=1)
        aux2_u_2 = np.concatenate((aux2_u_2,aux_u_2),axis=1)
    aux2_y = aux2_y[:,1:]
    aux2_u_1 = aux2_u_1[:,1:]
    aux2_u_2 = aux2_u_2[:,1:]
    phi = np.concatenate((aux2_y,aux2_u_0,aux2_u_1,aux2_u_2), axis=1)
    return y[ord:].copy(), phi

def estimate_output_fesc(y,u,theta,ord):
    y_hat = np.zeros(y.shape)
    for i in range(y.size-ord):
        for o in range(ord):
            y_hat[i+ord] += -theta[o]*y_hat[i+ord-1-o] + theta[o+ord+1]*u[i+ord-1-o] + theta[o+ord*2+1]*u[i+ord-1-o]**2
        y_hat[i+ord] += theta[ord]
    return y_hat