...
#include "../libs/gpio.h"
#include "../libs/uart.h"
#include "../libs/cossenos.h"   // sinal de entrada
...
#define TOTAL_TIME_15S   2*(750  + 10)
#define TOTAL_TIME_30S   2*(1500 + 10)
#define TOTAL_TIME_60S   2*(3000 + 10)
#define TOTAL_TIME_120S  2*(6000 + 10)
#define TOTAL_TIME_130S  2*(6500 + 10)
#endif
#ifdef DEGRAU
#define TOTAL_TIME  TOTAL_TIME_15S 
#endif
#ifdef NSENOS
#define TOTAL_TIME  TOTAL_TIME_60S 
#endif

#ifdef T20MS
#define SAMPLE_TIME 10// x2ms = 20.48ms
#define TOTAL_TIME_15S   750+10 
#define TOTAL_TIME_30S   1500+10
#define TOTAL_TIME_60S   3000+10
#define TOTAL_TIME_120S  6000+10
#define TOTAL_TIME_130S  6500+10
#endif

#ifdef ESCADA
    #define RAMP_STEP   4 
    #define RAMP_TIME   TIME_4S
#endif
...
void cb_update(GPT_t* drv){ // ciclo de 2.06 milissegundos
    ...
    if (cnt == SAMPLE_TIME) {
        ...
        #endif
        #ifdef NSENOS 
        entrada = NSines(tempo);                // segundo teste entrada variável
        gpt_enable_pwm_channel(GPTD3, 0, entrada); 
        #endif
        #ifdef ESCADA
        cnt3++;
        if ( cnt3 == 200 && entrada < PWM_MAX){
            entrada = entrada + 4; 
            gpt_enable_pwm_channel(GPTD3, 0, entrada);
            cnt3 = 0;
        }
...
ISR(PCINT1_vect) {
    ...
    #ifdef HALL_CNT
    if( (new_value & (1 << PINC0)) != (old_value & (1 << PINC0)) ) { // ~chg & (1 << PINC0) ){ // mudança no pino 0 da porta C
        hall++;
    }
    #endif
    #ifdef VEL_INST
